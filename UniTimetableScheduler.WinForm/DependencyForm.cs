    ...
    public partial class DependencyForm : Form
    {
        ...
        private void deleteAllButton_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                sqlQuery = @"DELETE FROM [Dependency]";
                SQLiteCommand cmd = new SQLiteCommand(sqlQuery, con);
                cmd.ExecuteNonQuery();
        ...
        private void printButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string fileName = "DependencyData.txt";
        ...
        public void ExportDataToTxt(DataTable data, string fileName)
        {
            try
            {
                ...
                {
                    // Write header
                    writer.WriteLine("1st SchedulerID, 2nd SchedulerID, InHowManyDays");

                    // Write rows
                    foreach (DataRow row in data.Rows)
                    {
                        writer.WriteLine($"{row["CourseID"]}, {row["DependentCourseID"]}, {row["InHowManyDays"]}");
                    }
                }

                MessageBox.Show($"Dependency data has been exported to the file '{fileName}' on the desktop successfully.");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"An error occurred: {ex.Message}");
